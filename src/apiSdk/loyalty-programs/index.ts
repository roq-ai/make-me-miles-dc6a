...
export const getLoyaltyPrograms = async (query?: LoyaltyProgramGetQueryInterface) => {
  const response = await axios.get(`/api/loyalty-programs${query ? `?${queryString.stringify(query)}` : ''}`);
