...
function OrganizationListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<OrganizationInterface[]>(
    ...
    () =>
      getOrganizations({
        relations: ['user', 'loyalty_program.count', 'loyalty_program_manager.count'],
      }),
  );
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  ...
                  {hasAccess('loyalty_program_manager', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>loyalty_program_manager</Th>
                  )}
                  <Th>Actions</Th>
                </Tr>
              </Thead>
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('loyalty_program', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.loyalty_program}</Td>
                    )}
                    {hasAccess('loyalty_program_manager', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.loyalty_program_manager}</Td>
                    )}
                    <Td>
